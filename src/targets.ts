...
export class TargetRegistry {
  ...
  register(target: TargetDefinition): void { if (this.targets.has(target.targetVersion)) throw new Error(`Duplicate target version: ${target.targetVersion}`); if (!Number.isInteger(target.horizonBars) || target.horizonBars <= 0) throw new Error("Target horizon must be positive"); this.targets.set(target.targetVersion, structuredClone(target)); }
