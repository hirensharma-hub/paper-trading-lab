...
export class TargetRegistry {
  ...
  register(target: TargetDefinition): void { if (this.targets.has(target.targetVersion)) throw new Error(`Duplicate target version: ${target.targetVersion}`); if (!Number.isInteger(target.horizonBars) || target.horizonBars <= 0) throw new Error("Target horizon must be positive"); if (target.timeBasis !== undefined && !(target.timeBasis === "TRADING_BARS" || target.timeBasis === "WALL_CLOCK")) throw new Error("Unknown target time basis"); if (target.entryReferenceMethod && !["NEXT_BAR_OPEN", "NEXT_QUOTE", "PAPER_FILL_REFERENCE"].includes(target.entryReferenceMethod)) throw new Error("Unknown entry reference method"); if (target.kind === "TRIPLE_BARRIER" && (!Number.isFinite(target.upperBarrierMultiple) || !Number.isFinite(target.lowerBarrierMultiple))) throw new Error("Triple-barrier target requires barrier multiples"); this.targets.set(target.targetVersion, structuredClone({ ...target, timeBasis: target.timeBasis ?? "TRADING_BARS" })); }
