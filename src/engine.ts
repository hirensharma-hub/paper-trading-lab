...
export class ResearchEngine {
  ...
  onBar(bar: Bar, quote?: Quote): Signal {
    ...
    if (this.operationalNowMs !== undefined && this.operationalNowMs - quote.ts > this.maxQuoteAgeMs) return { action: "HOLD", reason: "Market data is stale" };
  ...
  get health() { const now = this.operationalNowMs; const quoteTimestamps = Object.fromEntries(this.lastQuoteTs); const staleSymbols = now === undefined ? [] : [...this.lastQuoteTs.entries()].filter(([, ts]) => now - ts > this.maxQuoteAgeMs).map(([symbol]) => symbol); return { paused: this.paused || this.killSwitch, killSwitch: this.killSwitch, dataFresh: staleSymbols.length === 0, staleSymbols, lastQuoteTs: quoteTimestamps, symbols: [...this.histories.keys()], bars: [...this.histories.values()].reduce((sum, h) => sum + h.length, 0), balance: this.broker.balance, positions: this.broker.openPositions }; }
}
