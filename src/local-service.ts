...
export interface LocalServiceOptions { host?: string; port?: number; maxBodyBytes?: number; allowedOrigins?: readonly string[]; }
...
export interface LocalServiceStatus { ok: true; service: "paper-trading-lab"; safety: "PAPER_ONLY"; engine: unknown; }
...
function permittedOrigin(origin: string | undefined, configured: readonly string[]): string | undefined { if (!origin) return undefined; if (configured.includes(origin)) return origin; try { const parsed = new URL(origin); return parsed.protocol === "chrome-extension:" && Boolean(parsed.hostname) ? origin : undefined; } catch { return undefined; } }
...
export class LocalPaperEngineService {
  ...
  async start(): Promise<void> {
    ...
    this.server = createServer(async (request, response) => {
      ...
      try {
        if (request.headers.origin && !origin) { json(response, 403, { ok: false, error: "Origin is not allowed" }); return; }
        ...
        if (request.method === "GET" && path === "/health") { json(response, 200, { ok: true, service: "paper-trading-lab", safety: "PAPER_ONLY", engine: this.engine.health } satisfies LocalServiceStatus, origin); return; }
        if (request.method === "GET" && path === "/state") { json(response, 200, { ok: true, snapshot: this.engine.portfolioSnapshot(Date.now()), health: this.engine.health }, origin); return; }
      ...
      } catch (error) { json(response, 400, { ok: false, error: error instanceof Error ? error.message : "Request failed" }, origin); }
    });
