...
function dataStatus(health: Record<string, unknown>): "NO_DATA" | "FRESH" | "STALE" { return health.dataFresh ? "FRESH" : health.lastQuoteTs && Object.keys(health.lastQuoteTs as object).length ? "STALE" : "NO_DATA"; }
  ...
  async start(): Promise<void> {
    ...
    this.server = createServer(async (request, response) => {
      ...
      try {
        ...
        json(response, 404, { ok: false, error: { code: "NOT_FOUND", message: "Not found" } }, origin);
