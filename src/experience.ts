...
export interface PredictionResolution { resolutionTimestamp: number; forwardReturn: number; label: "WIN" | "LOSS" | "FLAT" | "AMBIGUOUS"; maxFavorableExcursion: number; maxAdverseExcursion: number; }
...
defaultTargetRegistry.register({ targetVersion: "forward-close-v1", kind: "FORWARD_CLOSE_RETURN" });
defaultTargetRegistry.register({ targetVersion: "triple-barrier-v1", kind: "TRIPLE_BARRIER", upperBarrierMultiple: 1, lowerBarrierMultiple: 1 });
...
export function resolvePrediction(prediction: PredictionRecord, bars: readonly Bar[], registry: TargetRegistry = defaultTargetRegistry): ExperienceRecord | null {
  const definition = registry.get(prediction.targetVersion); if (!definition || prediction.horizonBars <= 0 || (definition.horizonBars !== undefined && definition.horizonBars !== prediction.horizonBars)) return null;
