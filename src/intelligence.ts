...
import type { OodStatus, ScalerState } from "./ml";

export type IntelligenceDecision = "BUY" | "SELL" | "HOLD" | "NO_TRADE";
export interface PredictiveModel { predictProbability(features: readonly number[]): number; transformFeatures?(features: readonly number[]): readonly number[]; assessOod?(features: readonly number[]): { status: OodStatus; maxAbsZ: number }; }
...
export class IntelligenceEngine {
  ...
  analyze(bar: Bar, quote?: Quote): MarketAnalysisSnapshot {
    ...
    const structure = detectMarketStructure(next); const regime = classifyRegime(features, structure); const patterns = detectPatterns(features, structure, regime); const rawModelFeatures = featureVectorForModel(features); const modelFeatures = this.config.model?.transformFeatures?.(rawModelFeatures) ?? rawModelFeatures; const prediction = this.config.model ? { probability: this.config.model.predictProbability(rawModelFeatures), ood: this.config.model.assessOod?.(modelFeatures) } : undefined;
    const analogues = this.config.analogues?.length ? nearestAnalogues(modelFeatures, this.config.analogues, 50, this.config.minimumAnalogueSample ?? 20, { asOfTimestamp: decisionTimestamp, scaler: this.config.analogueScaler, requiredRegime: regime.trend }) : undefined;
