...
export interface PredictiveModel { predictProbability(features: readonly number[]): number; assessOod?(features: readonly number[]): { status: OodStatus; maxAbsZ: number }; }
...
export interface IntelligenceConfig { model?: PredictiveModel; analogues?: readonly AnalogueObservation[]; minimumAnalogueSample?: number; probabilityThreshold?: number; featureConfig?: { sessionsPerYear?: number; sessionMinutesPerDay?: number }; }
...
export class IntelligenceEngine {
  ...
  analyze(bar: Bar, quote?: Quote): MarketAnalysisSnapshot {
    ...
    const timestamp = bar.startMs + bar.intervalMs; const features = buildFeatures(next, quote, { ...this.config.featureConfig, decisionTimestamp: quote?.ts ?? timestamp });
    ...
    const structure = detectMarketStructure(next); const regime = classifyRegime(features, structure); const patterns = detectPatterns(features, structure, regime); const modelFeatures = featureVectorForModel(features); const prediction = this.config.model ? { probability: this.config.model.predictProbability(modelFeatures), ood: this.config.model.assessOod?.(modelFeatures) } : undefined;
    const analogues = this.config.analogues?.length ? nearestAnalogues(modelFeatures, this.config.analogues, 50, this.config.minimumAnalogueSample ?? 20) : undefined;
    const evidence = analogues ? assessEvidence({ sampleSize: analogues.sampleSize, outOfSample: true, calibrated: prediction !== undefined, costSurvives: true, regimeConsistent: !regime || !analogues.regimeDistribution[regime.trend] || analogues.regimeDistribution[regime.trend] >= 0.2, parameterStable: false, recentStable: false }) : undefined;
    ...
    const ood = prediction?.ood?.status; const decision: IntelligenceDecision = !prediction || !analogues || evidence?.quality === "INSUFFICIENT" || ood === "OUT_OF_DISTRIBUTION" ? "NO_TRADE" : prediction.probability >= (this.config.probabilityThreshold ?? 0.6) && (ev ?? -Infinity) > 0 ? "BUY" : "HOLD";
