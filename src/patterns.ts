...
export function detectPatterns(features: FeatureVector, structure: MarketStructureSnapshot | null, regime?: RegimeState): readonly PatternDetection[] {
  ...
  const breakoutStrength = (distance: number) => Math.min(1, Math.max(0, distance / Math.max(features.close, 1e-9) * 100)); const currentHigh = features.currentHigh ?? features.close; const currentLow = features.currentLow ?? features.close;
  ...
  if (currentHigh > structure.rangeHigh && features.close <= structure.rangeHigh) result.push({ ...base, type: "FAILED_BREAKOUT", strength: breakoutStrength(structure.rangeHigh - features.close), levels: { rangeHigh: structure.rangeHigh }, supportingFeatures: ["current_high_above_prior_range_high", "returned_below_prior_range_high"], contradictingFeatures: ["breakout_follow_through"] });
  if (currentLow < structure.rangeLow && features.close >= structure.rangeLow) result.push({ ...base, type: "FAILED_BREAKOUT", strength: breakoutStrength(features.close - structure.rangeLow), levels: { rangeLow: structure.rangeLow }, supportingFeatures: ["current_low_below_prior_range_low", "returned_above_prior_range_low"], contradictingFeatures: ["breakout_follow_through"] });
