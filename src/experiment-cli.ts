...
import { assertFiniteArtifact, canonicalJson, sanitizeArtifact, sha256CanonicalJson } from "./serialization";
...
function writeArtifacts(directory: string, report: ReturnType<ExperimentRunner["syntheticSmoke"]>, inputConfig?: ExperimentConfig & { manifest: DatasetManifest }, overwrite = false): void {
  ...
  const files: Record<string, unknown> = {
    ...
    "reproducibility-metadata": { gitCommit: process.env.GIT_COMMIT ?? "unknown", nodeVersion: versions.node, experimentRunnerVersion: "paper-trading-lab-0.1.0", seed: report.config?.seed, datasetHash: report.manifest?.contentHash, calendarHash: report.manifest?.calendarSpecHash, featureSetVersion: report.manifest?.featureSetVersion, targetVersion: report.manifest?.targetVersion, labelPolicyVersion: report.manifest?.labelPolicyVersion }
  };
  ...
  writeFileSync(`${directory}/experiment-report.json`, canonicalJson({ ...report, artifactManifest: manifest }));
}
