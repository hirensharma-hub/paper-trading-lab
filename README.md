...
The persisted experiment includes the complete analogue database (UP, DOWN, TIMEOUT and AMBIGUOUS), TRAIN-only analogue pool/scaler, exact model artifact, actual final-row ranges, cost model, evidence context, decision/exit/risk/broker/calendar configuration, metric frequency, regime-evidence setting and gap policy. Excluded labels never receive a fabricated binary label. Replays report warmup/test counts, order counts, fees, estimated slippage, action counts and metric quality; benchmarks use the same TEST interval and paper-cost path for the candidate, equal-weight buy-and-hold and EMA baseline. Synthetic mode is deterministic and intentionally permissive only to prove that the offline BUY/SELL path can run. A candidate is never auto-promoted.
