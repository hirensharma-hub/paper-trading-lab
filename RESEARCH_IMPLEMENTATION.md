...
| Offline experiment pipeline | Permitted offline research | CSV loader, manifest/hash/calendar-aware quality gates, point-in-time observations, immutable next-open target, explicit split purge, TRAIN-only model and analogue fitting, validation/calibration/untouched TEST diagnostics, outcome-preserving analogue database, frozen runtime manifest, cost-aware replay/benchmarks, honest reports and synthetic/real CLI | IMPLEMENTED; historical validation pending permitted data |
