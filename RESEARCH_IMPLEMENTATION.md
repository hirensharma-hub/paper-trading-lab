...
| Offline experiment pipeline | Permitted offline research | CSV loader, manifest/hash/full-session calendar-aware quality gates, point-in-time observations, immutable trading-bar next-open target, configured-boundary purge, TRAIN-only model/scaler and calibration fit ranges, validation/calibration/untouched TEST diagnostics, post-test OOS availability, outcome-preserving analogue database, frozen runtime manifest, delayed-intent cost-aware replay/benchmarks, actual-runtime EV/reason codes, explicit replay phases and settlement separation, typed execution audit evidence, cryptographic calendar provenance, readiness report and synthetic/real CLI | IMPLEMENTED; historical validation pending permitted data |
