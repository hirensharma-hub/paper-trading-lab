...
- Same-timestamp replay is atomic: opening marks, session baselines, pending next-open execution, all closing marks, completed-bar decisions, and one portfolio snapshot per timestamp are deterministic and covered by multi-symbol close-state and rename-invariance regressions.
...
- Runtime feature parity is authoritative for every expected comparable TEST observation and records expected, checked, missing, mismatch, IDs, maximum absolute difference, feature version, and feature IDs; missing runtime vectors fail readiness.
...
Result: 106 tests passed, 0 failed; both TypeScript checks pass; the clean dependency install, smoke CLI, full temporary fixture workflow, and GitHub Actions workflow pass. The temporary fixture is explicitly synthetic and its historical readiness is false.
