...
const testResolutionContext = { barSchedule: new ExpectedBarClock(new TradingCalendar({ timeZone: "UTC", sessionOpenHour: 0, sessionOpenMinute: 0, sessionCloseHour: 23, sessionCloseMinute: 59 }), 60_000), calendarSpecVersion: "test-calendar-v1", calendarSpecHash: "test-hash" };
...
class FixedDecisionEngine extends DecisionEngine {
  ...
  override decide(snapshot: MarketAnalysisSnapshot) { return { action: this.fixedAction, symbol: snapshot.symbol, timestamp: snapshot.timestamp, allowed: true, reasonCodes: [], humanReadableReason: "test action", reason: "test action" }; }
}
...
class SequenceDecisionEngine extends DecisionEngine {
  ...
  override decide(snapshot: MarketAnalysisSnapshot) { const action = this.actions[Math.min(this.index++, this.actions.length - 1)] ?? "NO_TRADE"; return { action, symbol: snapshot.symbol, timestamp: snapshot.timestamp, allowed: true, reasonCodes: [], humanReadableReason: "sequence", reason: "sequence" }; }
}
...
test("historical analogues and evidence quality expose sample size instead of fake certainty", () => {
  ...
  const passed = { reportId: "r", status: "PASSED" as const, sampleSize: 100, modelId: "m", modelVersion: "1", experimentId: "e", generatedAt: 100, availableAtTimestamp: 100 };
  assert.ok(assessEvidence({ sampleSize: 100, regimeConsistent: true, expectedProvenance: { modelId: "m", modelVersion: "1", experimentId: "e" }, context: { modelId: "m", modelVersion: "1", experimentId: "e", outOfSampleReport: passed, calibrationReport: passed, costStressReport: passed, walkForwardReport: passed, parameterStabilityReport: passed, recentStabilityReport: passed } }).score > 0.9);
});
...
test("metrics, intelligence, prediction resolution and model registry are integrated", () => {
  ...
  const prediction = resolvePrediction({ ...predictionState, predictionId: "p1", symbol: "SPY", decisionTimestamp: 20 * 60_000, horizonBars: 1, targetVersion: "forward-close-1-v1", probability: 0.7, decision: "BUY", modelVersion: "m1", featureVersion: "f1" }, [bar("SPY", 19, 100), { ...bar("SPY", 20, 105), open: 100 }], undefined, testResolutionContext);
...
test("model bundles, decision policy, target registry, queue and durable artifacts are reproducible", () => {
  ...
  const decision = new DecisionEngine({ minimumEvidence: "MODERATE" }).decide({ analysis: { symbol: "SPY", barCloseTimestamp: 1, decisionTimestamp: 1, timestamp: 1, features: null, structure: null, regime: null, patterns: [], evidence: { quality: "WEAK", score: 0.2, components: {}, provenance: {} } } }); assert.equal(decision.action, "NO_TRADE");
  ...
  const queue = new PredictionQueue(); queue.enqueue({ ...predictionState, predictionId: "q1", symbol: "SPY", decisionTimestamp: 20 * 60_000, horizonBars: 1, targetVersion: "forward-close-1-v1", probability: 0.7, decision: "BUY", modelVersion: "m1", featureVersion: "f1" }); assert.equal(queue.resolveAvailable([bar("SPY", 19, 100), bar("SPY", 20, 105)], undefined, testResolutionContext).length, 1);
  const directory = mkdtempSync(join(tmpdir(), "paper-lab-artifacts-")); try { const experiences = new JsonlExperienceRepository(join(directory, "experience.jsonl")); experiences.append(resolvePrediction({ ...predictionState, predictionId: "p2", symbol: "SPY", decisionTimestamp: 20 * 60_000, horizonBars: 1, targetVersion: "forward-close-1-v1", probability: 0.5, decision: "HOLD", modelVersion: "m1", featureVersion: "f1" }, [bar("SPY", 19, 100), bar("SPY", 20, 101)], undefined, testResolutionContext)!); assert.equal(experiences.all().length, 1); const artifacts = new JsonModelArtifactRepository(join(directory, "models.jsonl")); artifacts.append(artifact); assert.equal(artifacts.all()[0].artifactId, "a1"); const pending = new JsonPredictionQueueRepository(join(directory, "pending.jsonl")); pending.append({ ...predictionState, predictionId: "q2", symbol: "SPY", decisionTimestamp: 1, horizonBars: 1, targetVersion: "forward-close-1-v1", probability: 0.5, decision: "NO_TRADE", modelVersion: "m1", featureVersion: "f1" }); assert.equal(pending.all().length, 1); } finally { rmSync(directory, { recursive: true, force: true }); }
});
...
test("critical correctness gates enforce separation, schema, costs, OOD, and shared integrated replay", () => {
  ...
  const prepared = prepareDataset(rows); assert.throws(() => new LogisticRegression().unsafeFitRaw([...prepared.train, ...prepared.validation]));
  const oodSnapshot = { barCloseTimestamp: 1, decisionTimestamp: 1, symbol: "SPY", timestamp: 1, features: null, structure: null, regime: null, patterns: [], decision: "BUY" as const, reason: "test", evidence: { quality: "VERY_STRONG" as const, score: 1, components: {}, provenance: {} }, prediction: { predictionId: "p", modelId: "m", modelVersion: "1", featureSetVersion: "f", featureIds: [], targetVersion: "t", rawProbability: 0.9, calibratedProbability: 0.9, probability: 0.9, ood: { status: "OUT_OF_DISTRIBUTION" as const, maxAbsZ: 5 } } };
  assert.equal(new DecisionEngine({ minimumEvidence: "MODERATE" }).decide(oodSnapshot).action, "NO_TRADE");
  const model = { featureVersion: "wrong-schema", predictProbability: () => 0.9 }; const intelligence = new IntelligenceEngine({ model: model as unknown as PredictiveModelBundle, trustedAnalogues: [] }); let mismatch: ReturnType<IntelligenceEngine["analyze"]> | undefined; for (let index = 0; index < 20; index++) mismatch = intelligence.analyze(bar("SPY", index)); assert.equal(mismatch?.prediction, undefined);
  ...
  const triple = resolvePrediction({ ...triplePredictionState, predictionId: "tb", symbol: "SPY", decisionTimestamp: 60_000, horizonBars: 1, targetVersion: "triple-barrier-1-u1-d1-v1", targetParameters: { atr: 1 }, probability: 0.5, decision: "HOLD", modelVersion: "m", featureVersion: "f" }, [{ ...bar("SPY", 0, 100) }, { ...bar("SPY", 1, 100), high: 102, low: 100 }], undefined, testResolutionContext); assert.equal(triple?.resolved.label, "WIN");
});
...
test("integrated engine exhaustively maps BUY, SELL, HOLD, and NO_TRADE without fall-through", () => {
  ...
  const snapshot = { barCloseTimestamp: quote.ts, decisionTimestamp: quote.ts, symbol: "SPY", timestamp: quote.ts, features: null, structure: null, regime: null, patterns: [], decision: "HOLD" as const, reason: "valid", evidence: { quality: "STRONG" as const, score: 1, components: {}, provenance: {} }, prediction: { predictionId: "p", modelId: "m", modelVersion: "1", featureSetVersion: "f", featureIds: [], targetVersion: "t", rawProbability: 0.5, calibratedProbability: 0.5, probability: 0.5, ood: { status: "IN_DISTRIBUTION" as const, maxAbsZ: 0 } }, expectedValue: 0 } as MarketAnalysisSnapshot;
...
test("integrated portfolio marks include every symbol and replay updates MFE/MAE", () => {
  ...
  const snapshot = { barCloseTimestamp: 60_001, decisionTimestamp: 60_001, symbol: "SPY", timestamp: 60_001, features: null, structure: null, regime: null, patterns: [], decision: "HOLD" as const, reason: "valid", evidence: { quality: "STRONG" as const, score: 1, components: {}, provenance: {} }, prediction: { predictionId: "p", modelId: "m", modelVersion: "1", featureSetVersion: "f", featureIds: [], targetVersion: "t", rawProbability: 0.5, calibratedProbability: 0.5, probability: 0.5, ood: { status: "IN_DISTRIBUTION" as const, maxAbsZ: 0 } }, expectedValue: 0 } as MarketAnalysisSnapshot;
  const rthTestCalendar = new TradingCalendar({ timeZone: "UTC", sessionOpenHour: 0, sessionOpenMinute: 0, sessionCloseHour: 23, sessionCloseMinute: 59 });
...
test("triple-barrier resolution preserves AMBIGUOUS and target horizon authority", () => {
  const ambiguous = resolvePrediction({ ...triplePredictionState, predictionId: "amb", symbol: "SPY", decisionTimestamp: 60_000, horizonBars: 1, targetVersion: "triple-barrier-1-u1-d1-v1", targetParameters: { atr: 1 }, probability: 0.5, decision: "HOLD", modelVersion: "m", featureVersion: "f" }, [{ ...bar("SPY", 0, 100) }, { ...bar("SPY", 1, 100), high: 102, low: 98 }], undefined, { barSchedule: new ExpectedBarClock(new TradingCalendar({ timeZone: "UTC", sessionOpenHour: 0, sessionOpenMinute: 0, sessionCloseHour: 23, sessionCloseMinute: 59 }), 60_000), calendarSpecVersion: "test-calendar-v1", calendarSpecHash: "test-hash" }); assert.equal(ambiguous?.resolved.label, "AMBIGUOUS");
...
test("next-open intents recheck risk at a gap-up opening quote and apply spread once", () => {
  const snapshot = { barCloseTimestamp: 60_001, decisionTimestamp: 60_001, symbol: "SPY", timestamp: 60_001, features: null, targetState: { status: "AVAILABLE" as const, values: {}, featureVersions: [] }, structure: null, regime: null, patterns: [], decision: "BUY" as const, reason: "valid", evidence: { quality: "STRONG" as const, score: 1, components: {}, provenance: {} }, prediction: { predictionId: "candidate-p", modelId: "m", modelVersion: "1", featureSetVersion: "f", featureIds: [], targetVersion: "t", rawProbability: .9, calibratedProbability: .9, probability: .9, ood: { status: "IN_DISTRIBUTION" as const, maxAbsZ: 0 } } } as MarketAnalysisSnapshot;
  const broker = new PaperBroker({ initialCash: 10_000, feeBps: 0, slippageBps: 0 });
  ...
  const policy = { version: "next-open-v1", entryMethod: "NEXT_BAR_OPEN" as const, exitMethod: "NEXT_BAR_OPEN" as const };
  const engine = new IntegratedPaperResearchEngine(new FixedIntelligenceEngine(snapshot), new SequenceDecisionEngine(["BUY", "HOLD"]), broker, new RiskManager({ maxPositionValue: 1_000, maxGrossExposure: 1_000, maxDailyLoss: 1_000, maxDrawdown: 1_000, maxOrdersPerMinute: 1, feeBps: 0 }), "test", "1", calendar, undefined, undefined, undefined, undefined, policy, 100);
  const events = [{ bar: { ...bar("SPY", 0, 100), open: 100 }, quote: { symbol: "SPY", ts: 60_001, bid: 99, ask: 100, last: 100 } }, { bar: { ...bar("SPY", 1, 200), open: 200 }, quote: { symbol: "SPY", ts: 120_001, bid: 199, ask: 200, last: 200 } }];
  const replay = replayIntegrated(events, engine, { decisionWindowStart: 60_001, decisionWindowEnd: 120_001, outcomeDataEnd: 120_001 });
  assert.equal(replay.pendingIntentCount, 1); assert.equal(replay.executionRiskResizeCount, 1); assert.equal(replay.executionRiskRejectCount, 0); assert.ok(replay.phaseTimeline?.includes("WARMUP") === false); assert.ok(replay.phaseTimeline?.includes("TEST_DECISIONS")); assert.equal(replay.decisionWindowEndEquity, replay.snapshots.at(-1)?.equity); assert.equal(replay.settlementEquity, replay.finalEquity); assert.equal(replay.phaseTimeline?.at(-1), "SETTLEMENT"); assert.equal(replay.executionAudits?.length, replay.pendingIntentCount - replay.cancelledPendingIntentCount); assert.ok(replay.executionAudits?.every((audit) => audit.riskAllowed ? Boolean(audit.orderId) && audit.approvedQuantity > 0 : !audit.orderId && audit.approvedQuantity === 0)); assert.equal(replay.predictionCount, replay.testPredictionIds?.length ?? 0); assert.equal(replay.intentCreatedCount, 1); assert.equal(replay.intentExecutedCount, 1); assert.equal(replay.intentRejectedCount, 0); assert.equal(replay.intentExpiredCount, 0); assert.equal(replay.currentPendingCount, 0); assert.equal(replay.testActionCounts?.BUY, 1); assert.equal(replay.testActionCounts?.HOLD, 1); assert.equal(broker.allOrders.length, 1); assert.ok((broker.allOrders[0]?.quantity ?? 0) < 100); assert.ok(Math.abs((broker.allFills[0]?.price ?? 0) - 201) < 1e-9); assert.equal(broker.allOrders[0]?.submittedAt, 60_000);
  const cancelledBroker = new PaperBroker({ initialCash: 10_000, feeBps: 0, slippageBps: 0 }); const cancelledEngine = new IntegratedPaperResearchEngine(new FixedIntelligenceEngine(snapshot), new SequenceDecisionEngine(["BUY", "HOLD"]), cancelledBroker, new RiskManager({ maxPositionValue: 10_000, maxGrossExposure: 10_000, maxDailyLoss: 1_000, maxDrawdown: 1_000, maxOrdersPerMinute: 10, feeBps: 0 }), "test", "1", calendar, undefined, undefined, undefined, undefined, policy); const cancelled = replayIntegrated(events, cancelledEngine, { decisionWindowStart: 60_001, decisionWindowEnd: 60_001, outcomeDataEnd: 120_001, closeAtEnd: true }); assert.equal(cancelled.orderCount, 2); assert.equal(cancelled.cancelledPendingIntentCount, 0); assert.equal(cancelled.phaseTimeline?.at(-1), "SETTLEMENT"); assert.notEqual(cancelled.decisionWindowResult?.endMarkToMarketEquity, cancelled.settlementResult?.finalSettledEquity); assert.equal(cancelled.decisionWindowResult?.endMarkToMarketEquity, cancelled.decisionWindowEndEquity);
});
