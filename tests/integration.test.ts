...
test("next-open intents recheck risk at a gap-up opening quote and apply spread once", () => {
  const snapshot = { barCloseTimestamp: 60_001, decisionTimestamp: 60_001, symbol: "SPY", timestamp: 60_001, features: null, structure: null, regime: null, patterns: [], decision: "BUY" as const, reason: "valid", evidence: { quality: "STRONG" as const, score: 1, components: {}, provenance: {} }, prediction: undefined } as MarketAnalysisSnapshot;
  const broker = new PaperBroker({ initialCash: 10_000, feeBps: 0, slippageBps: 0 });
  ...
  assert.equal(replay.pendingIntentCount, 1); assert.equal(replay.executionRiskResizeCount, 1); assert.equal(replay.executionRiskRejectCount, 0); assert.ok(replay.phaseTimeline?.includes("WARMUP") === false); assert.ok(replay.phaseTimeline?.includes("TEST_DECISIONS")); assert.equal(replay.decisionWindowEndEquity, replay.snapshots.at(-1)?.equity); assert.equal(replay.settlementEquity, replay.finalEquity); assert.equal(replay.phaseTimeline?.at(-1), "SETTLEMENT"); assert.equal(replay.executionAudits?.length, replay.pendingIntentCount - replay.cancelledPendingIntentCount); assert.ok(replay.executionAudits?.every((audit) => audit.riskAllowed ? Boolean(audit.orderId) && audit.approvedQuantity > 0 : !audit.orderId && audit.approvedQuantity === 0)); assert.equal(broker.allOrders.length, 1); assert.ok((broker.allOrders[0]?.quantity ?? 0) < 100); assert.ok(Math.abs((broker.allFills[0]?.price ?? 0) - 201) < 1e-9); assert.equal(broker.allOrders[0]?.submittedAt, 60_000);
  const cancelledBroker = new PaperBroker({ initialCash: 10_000, feeBps: 0, slippageBps: 0 }); const cancelledEngine = new IntegratedPaperResearchEngine(new FixedIntelligenceEngine(snapshot), new SequenceDecisionEngine(["BUY", "HOLD"]), cancelledBroker, new RiskManager({ maxPositionValue: 10_000, maxGrossExposure: 10_000, maxDailyLoss: 1_000, maxDrawdown: 1_000, maxOrdersPerMinute: 10, feeBps: 0 }), "test", "1", calendar, undefined, undefined, undefined, undefined, policy); const cancelled = replayIntegrated(events, cancelledEngine, { decisionWindowStart: 60_001, decisionWindowEnd: 60_001, outcomeDataEnd: 120_001 }); assert.equal(cancelled.orderCount, 1); assert.equal(cancelled.cancelledPendingIntentCount, 0);
});
