...
test("historical analogues and evidence quality expose sample size instead of fake certainty", () => {
  ...
  const passed = { reportId: "r", status: "PASSED" as const, sampleSize: 100, modelId: "m", modelVersion: "1", experimentId: "e" };
...
test("next-open intents recheck risk at a gap-up opening quote and apply spread once", () => {
  ...
  const cancelledBroker = new PaperBroker({ initialCash: 10_000, feeBps: 0, slippageBps: 0 }); const cancelledEngine = new IntegratedPaperResearchEngine(new FixedIntelligenceEngine(snapshot), new SequenceDecisionEngine(["BUY", "HOLD"]), cancelledBroker, new RiskManager({ maxPositionValue: 10_000, maxGrossExposure: 10_000, maxDailyLoss: 1_000, maxDrawdown: 1_000, maxOrdersPerMinute: 10, feeBps: 0 }), "test", "1", calendar, undefined, undefined, undefined, undefined, policy); const cancelled = replayIntegrated(events, cancelledEngine, { decisionWindowStart: 60_001, decisionWindowEnd: 60_001, outcomeDataEnd: 120_001 }); assert.equal(cancelled.orderCount, 0); assert.equal(cancelled.cancelledPendingIntentCount, 1);
});
