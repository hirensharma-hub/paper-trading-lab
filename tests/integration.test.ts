...
test("next-open intents recheck risk at a gap-up opening quote and apply spread once", () => {
  ...
  assert.equal(replay.pendingIntentCount, 1); assert.equal(replay.executionRiskResizeCount, 1); assert.equal(replay.executionRiskRejectCount, 0); assert.equal(broker.allOrders.length, 1); assert.ok((broker.allOrders[0]?.quantity ?? 0) < 100); assert.ok(Math.abs((broker.allFills[0]?.price ?? 0) - 201) < 1e-9); assert.equal(broker.allOrders[0]?.submittedAt, 60_000);
