...
import { ExperimentRunner, validateManifest } from "../src/experiment";
...
test("split suggestion uses session boundaries and reserves a tail", () => { const suggestion = suggestSplits(bars, manifest()); const session = (ts: number) => Math.floor(ts / (30 * 60_000)); assert.equal(session(suggestion.trainEnd), session(suggestion.trainEnd)); assert.ok(suggestion.outcomeTailBars > 0); assert.ok(suggestion.testDecisionEnd < suggestion.outcomeDataEnd); });
