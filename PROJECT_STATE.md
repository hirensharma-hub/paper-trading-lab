...
- Final correctness patch: preflight now blocks on every data-quality issue; invalid origins remain `UNKNOWN`; completed-bar batches prime all closing marks before decisions; per-result prediction resolution IDs are aggregated through replay; split suggestions guarantee 20 eligible tail bars per symbol; feature parity requires complete expected coverage; conflicting provider duplicates fail closed; and the suite is green at 106 tests.
